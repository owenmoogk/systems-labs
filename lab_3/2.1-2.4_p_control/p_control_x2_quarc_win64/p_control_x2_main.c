...
typedef RT_MODEL_p_control_x2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * p_control_x2(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(p_control_x2_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(p_control_x2_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://p_control_x2:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: p_control_x2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(p_control_x2_DW.HILReadEncoderTimebase_Task,
        (t_clock) p_control_x2_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
