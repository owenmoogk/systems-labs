...
typedef RT_MODEL_open_loop_sine_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * open_loop_sine(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(open_loop_sine_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(open_loop_sine_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://open_loop_sine:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: open_loop_sine -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(open_loop_sine_DW.HILReadEncoderTimebase_Task,
        (t_clock) open_loop_sine_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
